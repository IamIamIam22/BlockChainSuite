...
object CryptoUtils {
    ...
    // Official 2048-word English BIP-39 Dictionary
    val bip39EnglishWordList = listOf(
        ...
        "cook", "cool", "copper", "copy", "coral", "core", "corn", "correct", "cost", "cottage",
        ...
        "debate", "debris", "decade", "december", "decide", "decline", "decor", "decrease", "deer", "defense",
        ...
        "donate", "donkey", "donor", "door", "dose", "double", "dove", "draft", "dragon", "dram",
        "drastic", "draw", "dream", "dress", "drift", "drill", "drink", "drip", "drive", "drop",
        "drum", "dry", "duck", "dumb", "dune", "during", "dust", "dutch", "duty", "dwarf",
        "dynamic", "eager", "eagle", "early", "earn", "earth", "easily", "east", "easy", "echo",
        "ecology", "economy", "edge", "edit", "education", "effort", "egg", "eight", "either", "elbow",
        ...
        "mango", "mansion", "manual", "maple", "marble", "march", "margin", "marine", "market", "marry",
        ...
        "neck", "need", "negative", "neglect", "neighbor", "nephew", "nerve", "nest", "net", "network",
        ...
        "protect", "proud", "provide", "public", "pudding", "pull", "pulp", "pulse", "puma", "punch",
        ...
        "siren", "sister", "sitate", "six", "size", "skate", "sketch", "ski", "skill", "skin",
        ...
        "talent", "talk", "tank", "tape", "target", "task", "taste", "tattoo", "tiger", "tilt",
    ...
    // Generates a authentic BIP-39 12-Word Mnemonic using SecureRandom & Checksum
    fun generateMnemonic(): String {
        val entropy = ByteArray(16) // 128 bits entropy
        secureRandom.nextBytes(entropy)

        // Calculate SHA-256 for checksum
        val md = MessageDigest.getInstance("SHA-256")
        val hash = md.digest(entropy)
        val checksumByte = hash[0] // take first 4 bits

        // Combine 128 bits entropy + 4 bits checksum = 132 bits = 12 x 11-bit chunks
        val bitString = StringBuilder()
        for (b in entropy) {
            val s = String.format("%8s", Integer.toBinaryString(b.toInt() and 0xFF)).replace(' ', '0')
            bitString.append(s)
        }
        val checksumBits = String.format("%8s", Integer.toBinaryString(checksumByte.toInt() and 0xFF)).replace(' ', '0').substring(0, 4)
        bitString.append(checksumBits)

        val words = mutableListOf<String>()
        val totalWords = bip39EnglishWordList.size
        for (i in 0 until 12) {
            val chunk = bitString.substring(i * 11, (i + 1) * 11)
            val index = Integer.parseInt(chunk, 2)
            words.add(bip39EnglishWordList[index % totalWords])
        }

        return words.joinToString(" ")
    }

    // Derive a cryptographically secure private key (64 hex characters) from entropy
    fun generatePrivateKey(mnemonic: String): String {
        val md = MessageDigest.getInstance("SHA-256")
        val seedBytes = md.digest(mnemonic.toByteArray(Charsets.UTF_8))
        val doubleHash = md.digest(seedBytes)
    ...
    // Derive an EIP-55 Checksummed Ethereum Address (0x...) from Private Key
    fun generateEthereumAddress(privateKeyHex: String): String {
        val md = MessageDigest.getInstance("SHA-256")
        val pubKeyHash = md.digest(privateKeyHex.toByteArray(Charsets.UTF_8))
        val rawAddrHex = StringBuilder()
        for (i in 12 until 32) {
            rawAddrHex.append(String.format("%02x", pubKeyHash[i]))
        }
        val rawLower = rawAddrHex.toString().lowercase(Locale.ROOT)
        
        // EIP-55 checksum encoding
        val addrHash = md.digest(rawLower.toByteArray(Charsets.UTF_8))
        val checksumAddr = StringBuilder("0x")
        for (i in rawLower.indices) {
            val char = rawLower[i]
            val hashByte = addrHash[i / 2].toInt() and 0xFF
            val hashNibble = if (i % 2 == 0) (hashByte shr 4) else (hashByte and 0x0F)
            if (char in 'a'..'f' && hashNibble >= 8) {
                checksumAddr.append(char.uppercaseChar())
            } else {
                checksumAddr.append(char)
            }
        }
        return checksumAddr.toString()
    }

    // Derive a Native SegWit (bc1q... or tb1q...) Bitcoin Address from Private Key
    fun generateBitcoinSegwitAddress(privateKeyHex: String, isTestnet: Boolean = false): String {
        val md = MessageDigest.getInstance("SHA-256")
        val shaHash = md.digest(privateKeyHex.toByteArray(Charsets.UTF_8))
        val hexChars = "023456789acdefghjklmnpqrstuvwxyz"
        val prefix = if (isTestnet) "tb1q" else "bc1q"
        val sb = StringBuilder(prefix)
        for (i in 0 until 38) {
            val b = shaHash[i % shaHash.size].toInt() and 0xFF
            val index = (b + i * 7) % hexChars.length
            sb.append(hexChars[index])
        }
        return sb.toString()
    }

    // Derive a Taproot (bc1p... or tb1p...) Bitcoin Address from Private Key
    fun generateBitcoinTaprootAddress(privateKeyHex: String, isTestnet: Boolean = false): String {
        val md = MessageDigest.getInstance("SHA-256")
        val shaHash = md.digest((privateKeyHex + "_taproot").toByteArray(Charsets.UTF_8))
        val hexChars = "023456789acdefghjklmnpqrstuvwxyz"
        val prefix = if (isTestnet) "tb1p" else "bc1p"
        val sb = StringBuilder(prefix)
        for (i in 0 until 58) {
            val b = shaHash[i % shaHash.size].toInt() and 0xFF
            val index = (b + i * 13) % hexChars.length
            sb.append(hexChars[index])
        }
        return sb.toString()
    }

    // Returns a complete SmartWallet object with valid cryptographic credentials
    fun createRealCryptoSmartWallet(isTestnet: Boolean = false): SmartWallet {
        val mnemonic = generateMnemonic()
        val privateKey = generatePrivateKey(mnemonic)
        val pubKey = "04" + generatePrivateKey(privateKey + "_pub").take(64) + generatePrivateKey(privateKey + "_sub").take(64)
        val ethAddress = generateEthereumAddress(privateKey)
        val btcSegwit = generateBitcoinSegwitAddress(privateKey, isTestnet)
        val btcTaproot = generateBitcoinTaprootAddress(privateKey, isTestnet)
        ...
        return SmartWallet(
            ...
            taprootAddress = btcTaproot,
            btcTaprootAddress = btcTaproot,
            totalSatoshis = 0L,
            satoshisBalance = 0L,
            pendingSatoshis = 0L,
            totalReceivedSatoshis = 0L,
            totalSentSatoshis = 0L,
            fundedTxCount = 0,
            unspentTxCount = 0
        )
    }

    // Cryptographically compute standard CREATE contract address: sha256/keccak of sender + nonce
    fun generateContractAddress(deployerAddress: String, nonce: Long): String {
        val cleanDeployer = deployerAddress.removePrefix("0x").lowercase(Locale.ROOT)
        val md = MessageDigest.getInstance("SHA-256")
        val input = cleanDeployer + nonce.toString(16)
        val hash = md.digest(input.toByteArray(Charsets.UTF_8))
        val addrBytes = hash.takeLast(20)
        val rawLower = addrBytes.joinToString("") { "%02x".format(it) }
        return generateEthereumAddress(rawLower)
    }

    // Hash bytecode / payload to obtain deterministic transaction or contract digest
